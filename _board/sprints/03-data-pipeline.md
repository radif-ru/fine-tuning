...
| # | Риск | Смягчение |
|---|------|-----------|
| 1 | Неправильный формат данных (невалидный JSON) | Валидация на загрузке, понятные ошибки |
| 2 | Очень большие датасеты (не влезают в память) | Streaming mode, batch processing |
| 3 | Chat template отсутствует у tokenizer | Fallback на raw text, предупреждение |
